...
{
        // ── Fraud Detection ───────────────────────────────────────────────────────
    public static IEnumerable<RateLimitRule> FraudPolicies() =>
    [
        // POST /transactions/evaluate — service-to-service, high limit
        new RateLimitRule
        {
            PolicyName = "service-evaluate",
            Limit      = 500,
            Window     = TimeSpan.FromMinutes(1),
            Paths      = ["/api/v1/transactions/evaluate"],
            Methods    = ["POST"]
        },
        new RateLimitRule
        {
